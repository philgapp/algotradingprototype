class LongShort {
  constructor(API_KEY,API_SECRET){
    ...
    this.allStocks = ["AAPL", "FB", "AMZN", "GOOG", "GOOGL", "T", "TMUS", "CSCO", "F", "USO", "UCO", "GLD", "BABA", "SQQQ", "TSLA", "DOMO", "TLRY", "SQ", "MRO", "SNAP", "GM", "SPLA", "BA", "SUI", "SUN", "CGC", "SPWR", "NIO", "CAT", "MSFT", "PANW", "OKTA", "TWTR", "TM", "RTN", "ATVI", "GS", "BAC", "MS", "TWLO", "QCOM"]
  ...
  getTotalPrice(stocks){
    ...
    stocks.forEach(async (stock) => {
      proms.push(new Promise(async (resolve, reject) => {
        ...
        }).catch((err) => {writeToEventLog(err + ' in getTotalPrice');});
      }));
    });
    return proms;
  }
  ...
  getPercentChanges(allStocks){
    ...
    allStocks.forEach((stock) => {
      promStocks.push(new Promise(async (resolve, reject) => {
        ...
        }).catch((err) => {writeToEventLog(err + ' in getPercentChanges');});
        resolve();
      }));
    });
    return promStocks;
...
function runScript(){
  ...
  let API_KEY = 'PKZ44I2BO9AITQJ6WWHN';
  let API_SECRET = '8POL/SsQwOW0QCqEbwIxdaXfncemazjlYTta0e1B';
