class AlpacaCORS {
  constructor(config) {
    ...
    this.corsUrl = config.corsUrl || "http://localhost:8080";
  }
  
  // Helper functions
  httpRequest(method,args,body=undefined) {
    return fetch(`${this.corsUrl}/${this.baseUrl}/v2/${args}`, {
      method: method, 
      mode: 'cors', 
      headers: {
  ...
  dataHttpRequest(method,args,body=undefined) {
    return fetch(`${this.corsUrl}/https://data.alpaca.markets/v1/${args}`, {
      method: method, 
      mode: 'cors', 
      headers: {
        "APCA-API-KEY-ID": this.keyId,
