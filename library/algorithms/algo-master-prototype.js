import AlpacaCORS from './cors-api'
...
class AlgoMasterPrototype {
    constructor({API_KEY,API_SECRET,stocks}) {
        ...
        this.allStocks = stocks//['DOMO', 'TLRY', 'SQ', 'MRO', 'AAPL', 'GM', 'SNAP', 'SHOP', 'SPLK', 'BA', 'AMZN', 'SUI', 'SUN', 'TSLA', 'CGC', 'SPWR', 'NIO', 'CAT', 'MSFT', 'PANW', 'OKTA', 'TWTR', 'TM', 'RTN', 'ATVI', 'GS', 'BAC', 'MS', 'TWLO', 'QCOM'];
            ...
            if(this.timeToClose < (60000 * 15)) {
                ...
                await this.alpaca.getPositions().then(async (resp) => {
                    ...
                    resp.forEach((position) => {
                        writeToEventLog('15 mins to market close: '+position,true)
                        promClose.push(new Promise(async (resolve,reject) => {
                            var orderSide;
                            if(position.side == 'long') orderSide = 'sell';
                            else orderSide = 'buy';
                            writeToEventLog(position.qty,true)
                ...
                writeToEventLog('Not 15 minutes to market close yet. this.rebalance()',true)
    ...
    async rebalance(){
        writeToEventLog('Every minute rebalance(). Await this.rerank() first.',true)
        await this.rerank();
    ...
    async rank(){
        ...
        // TODO
        // Predict direction, performance, time frame using tech indicators
        // Store rolling historical data based on time frame for each stock
        // Track real results against prediction and come up with a 'reliability factor' scale -
        //  help to identify stocks that are solid over multiple trades (even both shorting and longing)
    }
    ...
    updatePositions() {
        ReactDOM.render('', document.getElementById("positions-log"))
        const positionsList = new Array()
        this.alpaca.getPositions().then((resp) => {
            resp.forEach((position) => {
                positionsList.push(
                    <div className="position-inst">
            <p className="position-fragment">{position.symbol}</p>
            <p className="position-fragment">{position.qty}</p>
            <p className="position-fragment">{position.side}</p>
            <p className="position-fragment">{position.unrealized_pl}</p>
          </div>
                );
            })
            ReactDOM.render(positionsList.map((key) => { return key }), document.getElementById("positions-log"))
        })
    }

    updateOrders() {
        ReactDOM.render('', document.getElementById("orders-log"))
        const ordersList = new Array()
        this.alpaca.getOrders({
            status: "open"
        }).then((resp) => {
            resp.forEach((order) => {
                ordersList.push(
                    <div className="order-inst">
            <p className="order-fragment">{order.symbol}</p>
            <p className="order-fragment">{order.qty}</p>
            <p className="order-fragment">{order.side}</p>
            <p className="order-fragment">{order.type}</p>
          </div>
                );
            })
            ReactDOM.render(ordersList.map((key) => { return key }), document.getElementById("orders-log"))
        })
    }
}
/*
...
export function killScript(){
    writeToEventLog("Killing script.");
    AlgoMasterPrototype.kill();
}
export function writeToEventLog(text,algoLog) {
    if (algoLog) {
        if(typeof text !== 'object') {
            algoLogArray.push(<p className="event-fragment">{text}</p>)
            ReactDOM.render(algoLogArray.map((key) => {
                return key
            }), document.getElementById("bottom-container"))
            console.log('Algo Log: ' + text)
        } else {
            console.log(text)
        }
    } else {
        if(typeof text !== 'object') {
            eventLog.push(<p className="event-fragment">{text}</p>)
            console.log('Event Log: ' + text)
        } else {
            console.log(text)
        }
        //ReactDOM.render(eventLog.map((key) => { return key }), document.getElementById("event-log"))
    }
}

//module.exports = AlgoMasterPrototype
export default AlgoMasterPrototype